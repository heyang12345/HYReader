...
// MARK: -- 四周间距
let ReadViewTopSpace:CGFloat = 40
let ReadViewBottomSpace:CGFloat = 40
let ReadViewLeftSpace:CGFloat = 20
let ReadViewRightSpace:CGFloat = 20
let ReadTextViewSpaceWH:CGFloat = 8

// MARK: -- 通知
let ReadChangeBGColorKey:String = "ReadChangeBGColorKey"
...
class ReadParser: NSObject {
    ...
    class func separateLocalURL(url:NSURL,complete:((isOK:Bool)->Void)?) {
        ...
        if !ReadKeyedIsExistArchiver(bookID, fileName: bookID) {
            ...
            dispatch_async(dispatch_get_global_queue(0, 0)) {
                ...
                // 阅读模型
                let readModel = ReadModel.readModelWithLocalBook(url)
                
                ReadModel.updateReadModel(readModel)
    ...
    /**
     ...
     - returns: 章节模型数组
     */
    class func separateContent(bookID:String,content:String) ->[ReadChapterListModel] {
    ...
    /// 通过阅读模型 转换章节list模型
    class func GetReadChapterListModelTransformation(readChapterModel:ReadChapterModel) ->ReadChapterListModel {
        
        let readChapterListModel = ReadChapterListModel()
        
        // 计算高度
        readChapterListModel.chapterHeight = ReadParser.parserReadContentHeight(readChapterModel.chapterContent, configure: ReadConfigureManger.shareManager, width: ScreenWidth - ReadViewLeftSpace - ReadViewRightSpace)
    ...
    /**
     根据字体类型获取字体
     */
    class func GetReadFont(fontType:ReadFont,fontSize:CGFloat) ->UIFont {
    ...
    class func GetReadViewFrame() ->CGRect {
        ...
        return CGRectMake(ReadViewLeftSpace, ReadViewTopSpace, ScreenWidth - ReadViewLeftSpace - ReadViewRightSpace, ScreenHeight - ReadViewTopSpace - ReadViewBottomSpace)
    }
    ...
    /**
     ...
     - returns: CTFrameRef
     */
    class func parserRead(content:String,configure:ReadConfigureManger,bounds:CGRect) ->CTFrameRef {
    ...
    /**
     ...
     - returns: 计算获得的内容高度
     */
    class func parserReadContentHeight(content:String,configure:ReadConfigureManger,width:CGFloat) ->CGFloat {
    ...
    /**
     ...
     - returns: 属性字典
     */
    class func parserAttribute(configure:ReadConfigureManger) ->[String:AnyObject] {
        
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineSpacing = configure.readSpaceLineH
        paragraphStyle.alignment = NSTextAlignment.Justified
        let font = ReadParser.GetReadFont(configure.readFont, fontSize: CGFloat(configure.readFontSize.floatValue))
